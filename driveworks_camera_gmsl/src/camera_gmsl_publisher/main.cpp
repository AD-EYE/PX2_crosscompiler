...
public:
    CameraGMSL(const po::variables_map args): args_(args)
    {
        ...
        resolution_ratio = args_["resolution_ratio"].as<float>();
        
        // Validate the ratio is between 0.0 and 1.0
        if (resolution_ratio <= 0.0f || resolution_ratio > 1.0f) {
            ROS_WARN("Invalid resolution ratio %.2f. Must be between 0.0 and 1.0. Using 1.0 (full resolution).", resolution_ratio);
    ...
    void publish()
    {
        ...
        const int TARGET_WIDTH = camera_properties_.resolution.x * resolution_ratio;  // Desired width
        const int TARGET_HEIGHT = camera_properties_.resolution.y * resolution_ratio; // Desired Height
...
int main(int argc, const char *argv[])
{
    ...
    desc.add_options()
        ...
        ("resolution_ratio", po::value<float>()-> default_value(1.0f), 
